...
function NovelMeta({ novel }: Pick<NovelHeaderProps, "novel">) {
  return (
    <div>
      ...
      {novel.originalTitle ? (
        <p className="text-body text-muted-foreground mt-1 font-medium">{novel.originalTitle}</p>
      ) : null}
      {novel.author ? (
        <p className="text-sm text-muted-foreground mt-0.5">By {novel.author}</p>
      ) : null}
      {novel.description ? (
        <p className="mt-3 text-sm text-muted-foreground max-w-3xl leading-relaxed whitespace-pre-wrap">
          {novel.description}
        </p>
      ) : null}
    </div>
...
type ReadingActionsProps = Pick<
  NovelHeaderProps,
  "novelId" | "chapters" | "chaptersPending" | "lastReadChapter" | "firstChapter"
>;
...
function ReadingActions({
  ...
  if (chaptersPending || chapters.length === 0) return null;
  ...
  return (
    <div className="pt-2 flex flex-wrap items-center gap-2">
      {continueChapter ? (
        ...
        >
          <BookOpen className="size-4" />
          <span>{lastReadChapter ? "Continue Reading" : "Read First Chapter"}</span>
          {lastReadChapter ? (
...
export function NovelHeader({
  ...
  const readingActionsProps = { novelId, chapters, chaptersPending, lastReadChapter, firstChapter };
  return (
