...
- **Residual Hanzi visibility:** Admin-only `getResidualHanziChapters({ novelId })` server function checks translated chapters using `findResidualSourceChars` and returns chapters with >2 residual Hanzi; rendered as an amber badge on novel detail chapter table rows. **Inline repair** (`repairResidualHanzi` in `worker.ts`, helpers in `residual-repair.ts`): runs on marked text before `restoreParagraphMarkers` when >2 residual — surgical span splice first (one JSON call translating extracted Hanzi runs, spliced by offset), whole-chunk retry only when any span >200 chars (passage-level failure); survivors are kept and surface via the badge.
